...
public class ToastUtils {
    ...
    public static void showShort(String toastString) {
        ...
        if(toast == null){
            ...
            toast = Toast.makeText(AppConfig.getContextObject(), toastString, Toast.LENGTH_SHORT);
        }else {
    ...
    public static void showLong(String toastString) {
        ...
        if(toast == null){
            // 如果Toast对象为空了，那么需要创建一个新的Toast对象
            toast = Toast.makeText(AppConfig.getContextObject(), toastString, Toast.LENGTH_LONG);
        }else {
            // 如果toast对象还存在，那么就不再创建新的Toast对象
            toast.setText(toastString);
        }
